...
local function removeTrackedCrate(crateName, crateData)
    ...
    local function runRemovalPass(targetData)
        local initiallyExists = crateStillExistsByName()
        if not initiallyExists and (not targetData or not targetData.unit) then
        ...
        if targetData and tryDestroyObject(targetData.unit, "tracked object") then
            return true
        end
    ...
    local removedNow = runRemovalPass(crateData)

    timer.scheduleFunction(function(args)
        local stillExists, existsSource = crateStillExistsByName()
        if not stillExists then
            if AirDropState.playerCrates[args.crateName] then
                AirDropState.playerCrates[args.crateName] = nil
                debugMsg("[CLEANUP] Follow-up confirmed crate absent and cleared tracking: " .. args.crateName)
            end
            return
        end

        local latestData = AirDropState.playerCrates[args.crateName] or args.crateData
        debugMsg("[CLEANUP] Follow-up removal attempt for crate still present via " .. tostring(existsSource) .. ": " .. args.crateName)

        if runRemovalPass(latestData) then
            AirDropState.playerCrates[args.crateName] = nil
            debugMsg("[SUCCESS] Follow-up despawned material crate: " .. args.crateName)
        else
            debugMsg("[ERROR] Follow-up failed to despawn crate: " .. args.crateName)
        end
    end, {crateName = crateName, crateData = crateData}, timer.getTime() + 1)
        ...
        for i, crateInfo in ipairs(selectedCrates) do
            if crateInfo.data and crateInfo.name then
                ...
                if removeTrackedCrate(crateInfo.name, crateInfo.data) then
                    ...
                    debugMsg("[ERROR] Initial despawn pass failed; follow-up removal already scheduled for: " .. crateInfo.name)
                end
            else
